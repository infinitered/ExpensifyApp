const {
    app, dialog, BrowserWindow, Menu, MenuItem, shell, ipcMain,
} = require('electron');
    ...
    return (
        app
            ...
            .then(() => {
                 ...
                 */
                if (__DEV__) {
                    console.log('Registering new-expensify:// protocol');
                ...
                webRequest.onHeadersReceived(validDestinationFilters, (details, callback) => {
                    ...
                    if (details.responseHeaders['content-security-policy']) {
                        details.responseHeaders['content-security-policy'] = _.map(details.responseHeaders['content-security-policy'], value => (value.startsWith('frame-ancestors') ? `${value} ${APP_DOMAIN}` : value));
                    }
                ...
                ipcMain.on(ELECTRON_EVENTS.LOCALE_UPDATED, (event, updatedLocale) => {
                    console.log(`Updated event to ${updatedLocale} - ${event}`);
            ...
            .then((browserWindowRef) => {
                loadURL(browserWindow).then(() => {
                    console.log('Loaded URL', browserWindowRef.webContents.getURL(), deeplinkUrl);
                    if (!deeplinkUrl) {
                        return;
                    }
